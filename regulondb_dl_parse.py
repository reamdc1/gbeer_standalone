...
def parse_regulonDB_file_and_store_results(outfolder, min_genes, url, download, experimental_only, organism_dict_for_recovery):
    #outfile = outfolder + 'operon_names_and_genes_unfiltered.txt'
    unfiltered_regulong_parsed_file = outfolder + 'operon_names_and_genes_unfiltered.txt'
    if download:
        print "Download"
    ...
    for operon_line in [i.strip().split('\t') for i in open(unfiltered_regulong_parsed_file).readlines()]:
